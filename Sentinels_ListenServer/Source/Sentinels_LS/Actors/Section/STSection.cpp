...
#include "Actors/Section/STSection.h"
...
void ASTSection::StartSpawnEnemy()
{
	if (!HasAuthority())
		return;
	...
	GetWorld()->GetTimerManager().SetTimer(SpawnHandle, this, &ASTSection::TrySpawnAI, 1.f, true);
}

void ASTSection::StopSpawnEnemy()
{
	for (int i = 0; i < SpawnInfos.Num(); i++)
	{
...
void ASTSection::DespawnAllEnemys()
{
...
void ASTSection::SpawnEnemy(int InfoIdx)
{
	USTAIPoolingWorldSubsystem* PoolingSystem = GetWorld()->GetSubsystem<USTAIPoolingWorldSubsystem>();
	...
	for (int i = 0; i < Info.SpawnRate; i++)
	{
		...
		if (Enemy)
		{
			...
			Enemy->Delegate_OnEnemyDied.AddDynamic(this, &ASTSection::OnEnemyDied);
...
bool ASTSection::IsInFrontalCone(const FVector& locationToCheck, const FVector& originLocation, const FVector& forwardVector, float angleDeg) const
{
...
bool ASTSection::GetSpawnNavLocationForPlayer(int playerIdx, int infoIdx, FNavLocation& OutLocation) const
{
	if (Players.Num() == 0)
		return false;
...
void ASTSection::OnEnemyDied(AActor* DiedEnemy)
{
	USTWorldSpawnSubsystem* SpawnSystem = GetWorld()->GetSubsystem<USTWorldSpawnSubsystem>();
	if (SpawnSystem)
	{
